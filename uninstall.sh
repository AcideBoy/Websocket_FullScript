...
pkill -f python_service > /dev/null 2>&1 || true

log_info "Removing installed files and configurations..."

# Remove main Acide directory
rm -rf /etc/Acide
...
sed -i '/Acide\/banner/d' /etc/default/dropbear 2>/dev/null

# Remove systemd service files
rm -f /etc/systemd/system/python_service.service
rm -f /etc/systemd/system/badvpn-udpgw@.service

# Remove binaries
rm -f /usr/local/bin/python_service /usr/bin/badvpn-udpgw /usr/local/bin/gum
...
# Remove scripts
for cmd in Acide asx menu create-account delete-account edit-banner edit-response \
...
# Remove packages that were specifically installed by Acide
# Note: Being conservative here - only removing packages that are primarily for Acide
packages_to_remove=(
  dropbear
)
...
log_success "Acide uninstalled successfully."
